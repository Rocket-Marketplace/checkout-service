...
export class RabbitMQService implements OnModuleInit, OnModuleDestroy {
  ...
  private async setupExchanges() {
    // Exchange para eventos de carrinho
  ...
  async publishCartEvent(eventType: string, data: any) {
    const routingKey = `cart.${eventType}`;
    const message = {
      eventType,
      data,
      timestamp: new Date(),
    ...
    console.log(`Published cart event: ${eventType}`);
  }

  async publishOrderEvent(eventType: string, data: any) {
